...
{
	switch (xj_key) {
	case 128: return PLAT_KEY_ESC;       
	case 8:   return PLAT_KEY_BACKSPACE; 
	case 130: return PLAT_KEY_TAB;       
	case 10:  return PLAT_KEY_ENTER;     
	case 132: return PLAT_KEY_CAPS;      
	case 133: return PLAT_KEY_SHIFT;     
	case 134: return PLAT_KEY_CTRL;      
	case 135: return PLAT_KEY_ALT;       
	case 136: return PLAT_KEY_F1;        
	case 137: return PLAT_KEY_F2;        
	case 138: return PLAT_KEY_F3;        
	case 139: return PLAT_KEY_F4;        
	case 140: return PLAT_KEY_F5;        
	case 141: return PLAT_KEY_F6;        
	case 142: return PLAT_KEY_F7;        
	case 143: return PLAT_KEY_F8;        
	case 144: return PLAT_KEY_F9;        
	case 145: return PLAT_KEY_F10;       
	case 146: return PLAT_KEY_F11;       
	case 147: return PLAT_KEY_F12;       
	case 149: return PLAT_KEY_NUML;      
	case 150: return PLAT_KEY_SCROLL;    
	default:  return (uint32_t)xj_key;   
	}
}
...
static void xj380_set_render_draw_color(plat_renderer_t renderer, uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
	if (renderer) {
		renderer->r = r;
		renderer->g = g;
...
static void xj380_render_rect(plat_renderer_t renderer, float x, float y, float w, float h)
{
	if (!renderer || !renderer->window)
		return;
	UINT32 color = make_rgba(renderer->r, renderer->g, renderer->b, renderer->a);
	xapi_DrawRect(renderer->window->handle, (UINT32)x, (UINT32)y,
		      (UINT32)(x + w), (UINT32)(y + h), color, false);
}
...
static void xj380_render_fill_rect(plat_renderer_t renderer, float x, float y, float w, float h)
{
	...
	UINT32 color = make_rgba(renderer->r, renderer->g, renderer->b, renderer->a);
	xapi_DrawRect(renderer->window->handle, (UINT32)x, (UINT32)y,
		      (UINT32)(x + w), (UINT32)(y + h), color, true);
}

static void xj380_render_texture(plat_renderer_t renderer, plat_texture_t texture,
				 float x, float y, float w, float h)
{
	if (!renderer || !texture || !renderer->window)
		return;
	...
	case TEX_IMAGE:
		...
		if (texture->data.image.filepath) {
			xapi_DrawPicture(handle, (UINT32)x, (UINT32)y,
					 (UINT32)w, (UINT32)h,
	...
	case TEX_TEXT:
		...
		if (texture->data.text.text) {
			UINT32 color = make_rgba(texture->data.text.r,
						 texture->data.text.g,
						 texture->data.text.b,
						 texture->data.text.a);
			xapi_DrawText(handle, (UINT32)x, (UINT32)y,
				      (WSTR)texture->data.text.text,
	...
	case TEX_PIXELS:
		...
		if (texture->data.pixel.pixels && texture->width > 0 && texture->height > 0) {
			...
			if (render_w == texture->width && render_h == texture->height) {
				...
				xapi_WriteBufferA(handle, (UINT32)x, (UINT32)y,
						  (UINT32)render_w, (UINT32)render_h,
						  (XCOLORA *)texture->data.pixel.pixels);
			} else {
				
				XCOLORA *scaled = (XCOLORA *)xapi_AllocateMemory(
					(UINT64)(render_w * render_h * sizeof(XCOLORA)));
				if (scaled) {
...
static plat_texture_t xj380_create_texture_from_surface(plat_renderer_t renderer, plat_surface_t *surface)
{
	if (!renderer || !surface)
		return NULL;
	...
	case SURF_IMAGE:
		...
		if (surface->data.image.filepath) {
			t->data.image.filepath = strdup(surface->data.image.filepath);
		}
		break;
	...
	case SURF_TEXT:
		...
		if (surface->data.text.text) {
			t->data.text.text = strdup(surface->data.text.text);
		}
...
static plat_texture_t xj380_create_texture(plat_renderer_t renderer,
...
{
	if (!texture || !pixels || texture->type != TEX_PIXELS)
		return -1;
...
static plat_surface_t *xj380_load_image(const char *filepath)
{
	...
	s->width = (int)w;
	s->height = (int)h;
	s->data.image.filepath = strdup(filepath);
...
static plat_surface_t *xj380_render_text_blended(plat_font_t font,
...
static plat_audio_device_t xj380_open_audio_device(plat_audio_format_t format, int channels, int freq)
{
	(void)format;
	(void)channels;
	(void)freq;
...
static plat_audio_stream_t xj380_open_audio_device_stream(plat_audio_format_t format, int channels, int freq)
{
	(void)format;
	(void)channels;
	(void)freq;
...
static int xj380_load_wav(const char *filepath, plat_audio_spec_t *spec, uint8_t **buffer, uint32_t *length)
{
	(void)filepath;
	(void)spec;
	(void)buffer;
