...
static int ensure_sound_stream(bapi_sound_t sound)
{
	...
	sound->stream = plat->create_audio_stream(&sound->spec);
	if (sound->stream == NULL) {
		printf("[AUDIO] Failed to create sound stream\n");
		return 1;
	}

	if (plat->bind_audio_stream(audio_device, sound->stream) != 0) {
		printf("[AUDIO] Failed to bind sound stream\n");
		plat->destroy_audio_stream(sound->stream);
		sound->stream = NULL;
		return 1;
	}

	if (plat->set_audio_stream_gain(sound->stream, sound->volume) != 0) {
...
static int queue_sound_buffer(bapi_sound_t sound)
{
	...
	if (plat->put_audio_stream_data(sound->stream, sound->buffer, (int)sound->length) != 0) {
		printf("[AUDIO] Failed to queue audio data\n");
		return 1;
	}

	if (plat->flush_audio_stream(sound->stream) != 0) {
...
int bapi_audio_init(void)
{
	...
	audio_device = plat->open_audio_device(PLAT_AUDIO_F32, 2, 44100);
...
void bapi_audio_cleanup(void)
{
	...
	while (sound != NULL) {
		...
		sound->queued_once = 0;
		if (sound->stream != NULL) {
			plat->clear_audio_stream(sound->stream);
		}
		sound->next_active = NULL;
		sound = next;
	}
	...
	if (audio_device != NULL) {
		plat->close_audio_device(audio_device);
		audio_device = NULL;
	}
}
...
bapi_sound_t bapi_sound_load(const char* filepath)
{
	...
	if (plat->load_wav(filepath, &sound->spec, &sound->buffer, &sound->length) != 0) {
	...
	printf("[AUDIO] Loaded WAV: channels=%d, freq=%d, length=%u\n",
	       sound->spec.channels, sound->spec.freq, sound->length);
...
void bapi_sound_set_volume(bapi_sound_t sound, float volume)
{
	if (sound != NULL) {
		...
		if (sound->stream != NULL) {
			plat->set_audio_stream_gain(sound->stream, sound->volume);
		}
	}
}
...
void bapi_sound_update(void)
{
	...
	while (sound != NULL) {
		...
		int queued = plat->get_audio_stream_queued(sound->stream);
		if (queued < 0) {
