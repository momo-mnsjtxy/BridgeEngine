...
bool bapi_log_init(const bapi_log_config_t* config)
{
	...
	if (plat) {
		g_log_state.mutex = plat->create_mutex();
	...
	if (plat) {
		plat->log_info("Logger initialized with level: %s",
...
void bapi_log_shutdown(void)
{
	...
	if (plat && g_log_state.mutex != NULL) {
		plat->destroy_mutex(g_log_state.mutex);
		      ...
		      ...)
{
	...
	if (plat && g_log_state.mutex != NULL) {
		plat->lock_mutex(g_log_state.mutex);
	}

	switch (level) {
	case BAPI_LOG_LEVEL_DEBUG:
		if (plat) plat->log_debug("%s", buffer);
		break;
	case BAPI_LOG_LEVEL_INFO:
		if (plat) plat->log_info("%s", buffer);
		break;
	case BAPI_LOG_LEVEL_WARN:
		if (plat) plat->log_warn("%s", buffer);
		break;
	case BAPI_LOG_LEVEL_ERROR:
		if (plat) plat->log_error("%s", buffer);
		break;
	case BAPI_LOG_LEVEL_CRITICAL:
		if (plat) plat->log_critical("%s", buffer);
		break;
	default:
		if (plat) plat->log_info("%s", buffer);
		break;
	...
	if (plat && g_log_state.mutex != NULL) {
		plat->unlock_mutex(g_log_state.mutex);
	}
}
