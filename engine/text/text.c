...
void bapi_text_init(void)
{
	...
	if (state->text_initialized) return;
	memset(g_font_cache, 0, sizeof(g_font_cache));
	state->text_initialized = 1;
}
...
void bapi_text_cleanup(void)
{
	...
	bapi_engine_state()->text_initialized = 0;
}
...
void bapi_draw_text(const char *text, float x, float y, float size, bapi_color_t color)
{
	...
	plat_surface_t *surface = plat->render_text_blended(font, text, (int)strlen(text), color.r,
														color.g, color.b, color.a);
	if (!surface) {
		return;
	}

	plat_texture_t texture = plat->create_texture_from_surface(renderer, surface);
	if (!texture) {
	...
	plat->render_texture(renderer, texture, x, y, (float)surf_w, (float)surf_h);
