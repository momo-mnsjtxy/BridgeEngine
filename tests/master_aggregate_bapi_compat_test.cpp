...
#define CHECK_SIGNATURE(symbol, type) static_assert(std::is_same<decltype(&symbol), type>::value, #symbol);
#define CHECK_MEMBER(type, member, expected) static_assert(std::is_same<decltype(((type *)0)->member), expected>::value, #type "." #member);
...
static_assert(std::is_same<decltype(&bapi_event_get_type), int (*)(const bapi_event_t *)>::value,
	"bapi_event_get_type");
static_assert(std::is_same<decltype(&bapi_event_get_key_code), uint8_t (*)(const bapi_event_t *)>::value,
	"bapi_event_get_key_code");
static_assert(std::is_same<decltype(&bapi_color_from_hex), bapi_color_t (*)(uint32_t)>::value,
	"bapi_color_from_hex");
static_assert(std::is_same<decltype(&bapi_scene_create), bapi_scene_t (*)(const char *, bapi_scene_callbacks_t)>::value,
	"bapi_scene_create");
static_assert(std::is_same<decltype(&bapi_level_create), bapi_level_t (*)(const char *, int, bapi_level_callbacks_t)>::value,
	"bapi_level_create");
static_assert(std::is_same<decltype(&bridgeengine_get_version_number), int (*)(void)>::value,
	"bridgeengine_get_version_number");
static_assert(BAPI_EVENT_QUIT == 0 && BAPI_EVENT_KEY_DOWN == 1 && BAPI_EVENT_MOUSE_BUTTON_DOWN == 2 &&
...
static_assert(BAPI_LOG_LEVEL_DEBUG == 0 && BAPI_LOG_LEVEL_INFO == 1 && BAPI_LOG_LEVEL_WARN == 2 &&
	BAPI_LOG_LEVEL_ERROR == 3 && BAPI_LOG_LEVEL_CRITICAL == 4 && BAPI_LOG_LEVEL_NONE == 5, "BAPI log values");
