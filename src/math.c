...
bapi_rect_t bapi_rect_intersection(bapi_rect_t a, bapi_rect_t b) {
    ...
    r.x = fmaxf(a.x, b.x);
    r.y = fmaxf(a.y, b.y);
    r.w = fminf(a.x + a.w, b.x + b.w) - r.x;
    r.h = fminf(a.y + a.h, b.y + b.h) - r.y;
    if (r.w < 0) r.w = 0;
    if (r.h < 0) r.h = 0;
    return r;
}
...
bapi_rect_t bapi_rect_union(bapi_rect_t a, bapi_rect_t b) {
    ...
    r.x = fminf(a.x, b.x);
    r.y = fminf(a.y, b.y);
    r.w = fmaxf(a.x + a.w, b.x + b.w) - r.x;
    r.h = fmaxf(a.y + a.h, b.y + b.h) - r.y;
    return r;
}
...
int bapi_circle_intersects_rect(bapi_circle_t c, bapi_rect_t r) {
    float near_x = fmaxf(r.x, fminf(c.x, r.x + r.w));
    float near_y = fmaxf(r.y, fminf(c.y, r.y + r.h));
    float dx = c.x - near_x;
    float dy = c.y - near_y;
    return (dx * dx + dy * dy) <= (c.r * c.r);
}
